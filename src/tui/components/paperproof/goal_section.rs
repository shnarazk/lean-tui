...
impl Component for GoalSection {
    ...
    fn render(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let lines: Vec<Line<'static>> = self.goals
            .iter()
            .enumerate()
            .map(|(goal_idx, goal)| {
                let is_selected = self.selection == Some(SelectableItem::GoalTarget { goal_idx });
                let is_spawned = goal.user_name.as_ref().is_some_and(|name| self.spawned_goal_ids.contains(name));
...
fn render_goal_line(goal: &Goal, is_selected: bool, is_spawned: bool, idx: usize, total: usize) -> Line<'static> {
    let state = DiffState {
    ...
    let marker = match (is_spawned, goal.is_inserted, goal.is_removed, goal.target.has_any_diff()) {
